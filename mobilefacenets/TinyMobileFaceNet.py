...
_CONV_DEFS = [
    ...
    InvResBlock(kernel=[3, 3], stride=2, depth=64, ratio=2, repeate=3),
    InvResBlock(kernel=[3, 3], stride=2, depth=128, ratio=4, repeate=1),
    InvResBlock(kernel=[3, 3], stride=1, depth=128, ratio=2, repeate=4),
    InvResBlock(kernel=[3, 3], stride=2, depth=128, ratio=4, repeate=1),
    InvResBlock(kernel=[3, 3], stride=1, depth=128, ratio=2, repeate=2),

    Conv(kernel=[1, 1], stride=1, depth=256, ratio=1),
]
    ...
    with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
      ...
      with tf.compat.v1.variable_scope('Logits'):
        if global_pool:
          ...
          net = slim.conv2d(inputs=net, num_outputs=256, kernel_size=[1, 1], stride=1, activation_fn=None, padding='VALID')
